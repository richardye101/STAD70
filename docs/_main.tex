...
We have a fitted mean \(\mu\) of \ensuremath{3.2240655\times 10^{-4}} and variance \(\sigma^{2}\) of \ensuremath{1.4635962\times 10^{-4}}.
This is compared to the sample mean \(\mu\) of \ensuremath{5.661532\times 10^{-4}} and variance \(\sigma^2\) of \ensuremath{1.3827261\times 10^{-4}}.
...
\paragraph{part (a)}\label{part-a-16}}
...
\paragraph{part (b)}\label{part-b-15}}
...
\paragraph{part (c)}\label{part-c-8}}
...
\end{gathered}
\]
Integral of a mixture model:
\[
\begin{gathered}
F_{X}(x) = P(X\leq x) = \int_{domain \ Y} \overbrace{ P(X<x| Y = y) }^{ \sim Exp(\lambda) = F_{X}(x) }f_{Y}(y) \, dy 
\end{gathered}
\]
